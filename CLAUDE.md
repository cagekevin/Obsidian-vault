...
| `作图经验/`（在 `Projects/HKH品牌/作图经验/`） | **我的作图方法论** — 白底产品照、风格定义、场景案例、协作流程 | **每次需要出图时，必须先读 `00-出图工作流.md`**，按流程执行。`instructions.md` 有文件清单和自进化规则。踩坑经验沉淀在 `场景案例/` |
...
**具体场景举例：**
- Kevin 要出图/做小红书/做产品图 → **先去读 `Projects/HKH品牌/作图经验/00-出图工作流.md`**，按流程执行
