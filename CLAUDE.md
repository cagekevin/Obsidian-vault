...
| `作图经验/`（在 `Projects/HKH品牌/作图经验/`） | **我的作图方法论** — 白底产品照、风格定义、场景案例、协作流程 | **每次出图前必须先读这里**。`instructions.md` 有文件清单和自进化规则。踩坑经验沉淀在 `场景案例/` |
...
**具体场景举例：**
- Kevin 要出图/做小红书/做产品图 → **先去读 `Projects/HKH品牌/作图经验/`**，按场景找对应方法论和踩坑记录
