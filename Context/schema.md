# Schema —— AI 知识加工规则

这个文档定义了我 vault 里信息的加工流程。**AI 每次处理新资料时，按这里的规则执行。**

---

## 一、文件类型

| 类型 | 对应文件夹 | 谁负责 | 说明 |
|------|-----------|--------|------|
| **原始资源** | `Clippings/raw/` | 人放入，AI 只读不改 | 视频转录稿、PDF 摘录等未经加工的原始材料 |
| **整理笔记** | `Clippings/`（根目录） | 人写 | 我对原始资料的整理和思考 |
| **知识图谱** | `Wiki/` | **AI 全权维护** | 从原始资料提取的原子化概念页面，互相链接 |
| **结构化知识** | `Reading/`、`Skills/` | 人+AI 协作 | 读书笔记、技能定义 |
| **规则文档** | `Context/schema.md` | 人+AI 协作优化 | 定义加工规则和流程 |
| **操作日志** | `Daily Notes/`、`Wiki/log.md` | AI 写 | 工作记录和操作日志 |
| **AI 记忆** | `.codebuddy/memory/` | **AI 全权维护** | 长期记忆（MEMORY.md）+ 短期记忆（YYYY-MM-DD.md） |

---

## 二、加工流程（Ingest → Wiki → Lint）

### 1. Ingest（摄入）

当新资料进入 `Clippings/raw/` 时，AI 按以下步骤处理：

1. 读原始资料的全文
2. 提取关键**实体**（人、公司、项目、工具）和**概念**（方法论、技术名词）
3. 检查 `Wiki/` 中是否已有相关页面：
   - 如果有 → 更新现有页面，补充新信息，添加双向链接
   - 如果没有 → 在 `Wiki/` 创建新页面（每个实体/概念一个文件）
4. 更新 `Wiki/index.md`（所有页面清单 + 一句话摘要）
5. 追加 `Wiki/log.md` 记录本次操作
6. 更新 `Daily Notes/` 记录

### 2. Query（问答）

当我提问时，AI 优先基于 `Wiki/` 的知识图谱回答，同时附带原始来源引用（`Clippings/raw/`）。如果 Wiki 中没有相关内容，再去 `Reading/` 和 `Skills/` 找。

高质量的回答可以回存到 `Wiki/` 作为新页面。

### 3. Lint（审查）

AI 定期（或我要求时）检查 `Wiki/`：

1. **矛盾检查**：同一概念在不同页面中是否有矛盾的表述
2. **孤立页面**：有没有没有与其他页面建立双向链接的页面
3. **缺失链接**：有没有提到了概念但没有创建对应页面
4. **过时信息**：有没有被新资料推翻的旧表述

---

## 三、命名规范

| 文件夹 | 命名规则 | 示例 |
|--------|---------|------|
| `Clippings/raw/` | 保持原文件名 | `raw-20260620-转录稿.txt` |
| `Clippings/` | `来源-日期-标题.md` | `twitter-20260620-Obsidian第二大脑.md` |
| `Wiki/` | `概念名.md`（简短精确的中文） | `LLM-Wikid框架.md` |
| `Reading/` | `书名-作者.md` 或 `主题-日期.md` | `深度工作-卡尔·纽波特.md` |
| `Daily Notes/` | `YYYY-MM-DD.md` | `2026-06-20.md` |
| `Projects/` | `项目名/`（中文，不加日期） | `HKH皮卡斯03版本/` |
| `.codebuddy/memory/` | `MEMORY.md`（长期）、`YYYY-MM-DD.md`（短期） | `MEMORY.md`、`2026-06-21.md` |

---

## 四、风格约定

- 内部链接使用 Obsidian 的双向链接规范 `[[页面名]]`
- Wiki/ 中的每个页面必须有 frontmatter 标注类型：`type: entity | concept | source-summary | comparison | synthesis`
- 区分 AI 生成和人工编写的内容

---

## 五、持续优化

这个 Schema 不是一次写死的。随着使用：
- 发现 AI 处理得不好的地方 → 修改 Schema
- 有了新的资料类型 → 补充到文件类型表
- 工作流变化 → 更新加工流程
