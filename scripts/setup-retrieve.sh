...
# Python runner: detect cross-platform (python3 on macOS, python on Windows, via uv)
# Priority: python3 -m uv (macOS) → python -m uv (Windows PowerShell) → python3/python (fallback)
PYTHON_CMD=""
for candidate in "python3 -m uv run python" "python -m uv run python" "python3" "python"; do
...
if [ -z "$PYTHON_CMD" ]; then
  echo "ERR: no Python found (tried: python3 -m uv, python -m uv, python3, python)" >&2
  exit 2
fi
...
set +e
$PYTHON_CMD "$VAULT/scripts/contextual-prefix.py" "${ARGS[@]}"
STAGE1_RC=$?
set -e
if [ "$STAGE1_RC" -ne 0 ]; then
  ...
  warn "  3. Re-process one page:  $PYTHON_CMD scripts/contextual-prefix.py wiki/<failing-page>.md --rebuild"
  exit 5
fi
...
$PYTHON_CMD "$VAULT/scripts/bm25-index.py" build

# ── 7. Smoke-test retrieve.py ────────────────────────────────────────────────
say ""
say "═══ Smoke test ═══"
SMOKE_OUT="$($PYTHON_CMD "$VAULT/scripts/retrieve.py" "wiki" --top 1 2>/dev/null || echo '{}')"
...
say "  $PYTHON_CMD scripts/retrieve.py \"your question here\" --top 5"
say ""
