**⚠️ 重要说明：此示例使用旧 catalog ID（`broll-hero.*` / `aroll.*` / `broll-charts.*`），当前 catalog 中已不存在。仅作分镜结构和字段格式的参考，不可直接套用组件 ID。当前 catalog 的组件 ID 请查阅 `references/components-catalog.md`。**
