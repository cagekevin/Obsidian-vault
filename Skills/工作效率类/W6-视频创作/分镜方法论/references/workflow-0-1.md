---
name: workflow-0-1
description: 0-1 模式工作流。从用户初次表达需求到生成 storyboard.md 的完整流程。
---

# 0-1 模式工作流

[使用时机]
    项目里没有 storyboard.md，用户第一次表达想做视频。
...
[完成后衔接文案]
    ✓ "素材清点完了——逐字稿你来出，参考素材我列了 4 条待搜。接下来定一下视频本身怎么讲，画面用什么手段。"
...
[围栏]
    ...
    - 用户敷衍"随便 / 看你的"→ 不接受，从 Phase 1 视频类型推荐 2-3 个方向让他三选一
    - 用户敷衍且想自定义 → 逼用户给三个形容词，你来出方案
    - 不允许"等做出来再调主题"——必须先定
...
[完成后衔接文案]
    ✓ "调子定了——暗色锐利主题，橙色做点缀，装饰走 medium。再补两个具体参考和几条反例，方向就齐了。"
...
## [分镜起草阶段]
    ...
    不许自创组件名——必须落到 catalog 登记过的真实 ID；
    找不到合适组件 → 用 `placeholder.tbd` 占位并登记。
...
## [自审环节——输出前 AI 自我审查]
    ...
    在输出 storyboard.md 给用户之前，必须做一轮自我审查。
    审查不是给用户看的报告，是 AI 内部自检：发现问题 → 自己改掉 → 再输出。

    [审查维度]

    1. **节奏检查**
       节奏曲线是否均速？最长镜头和最短镜头时长差 ≥ 3 倍，才不算平。
       连续同速镜头 ≥ 3 个 → 调整其中 1-2 个的时长，制造快慢对比。
       停顿段（最慢的 2 个镜头之间）是否有信息承载？没有就砍或加内容。

    2. **信息密度检查**
       是否有连续 3 个镜头在讲同一件事？→ 合并或砍掉 1 个。
       每个 Scene 的期待内容是否唯一且具体？
       "空帧"嫌疑：某个 Scene 的期待内容是空话（"展示画面"/"过渡"）→ 重写或标记 [待补充]。

    3. **组件多样性检查**
       同一组件（同一组件 ID）是否出现 ≥ 4 次？→ 考虑替换其中 1-2 镜为同类组件。
       同一类展现对象是否占 Scene 总数 ≥ 50%？→ 视觉单调，换 2-3 种其他类型。

    4. **运镜健康检查**
       固定镜头占比是否 ≥ 60%？低于则调整。
       推 / 拉 / 摇 / 移 各是否 ≤ 3 次？超限则砍。

    5. **反例对照**
       见下方 [反例对照] 章节。

    自审发现问题后直接修改 spec，不要保留"问题记录"给用户看。
    只有自审发现**重大冲突**（全片叙事方向问题、核心信息被稀释）时，才暂停输出并告知用户。

[输出阶段]

[输出规则]
    - 按 `templates/storyboard-template.md` 的骨架输出 storyboard.md
    - 填字段前读 `references/spec-rules.md`：字段约束 / 规格一致性校验 / 交付前自检清单
    - **输出前先执行 [自审环节] 进行自我审查，审查通过后再往下走**
    - 未达成的地方标注 [待补充]，不要凭空补
    - 分镜表每个 Scene 都要锚定到具体组件 ID
    - 时长精确到 0.1s
    - 输出前用 spec-rules.md 的 [交付前自检清单] 逐条过一遍
    - 素材路径用相对路径
    - 文件名 storyboard.md，全小写，放项目根目录

[反例对照]
    输出前必须把 spec 和 Phase 5 收集的反例逐条比对。

    操作方式：
    1. 取出 storyboard.md § 8 的「反例」和「同质化反例」清单
    2. 逐条检查 spec 是否撞上：
       - 视觉反例 → 检查各 Scene 的画面描述 / 屏显文案 / 视觉主题
       - 叙事反例 → 检查旁白 / 叙事节拍 / 情绪曲线
       - 节奏反例 → 检查各 Scene 时长 / 转场密度
    3. 若发现冲突：
       - 轻微冲突（局部配色 / 单镜头节奏）→ 直接改掉，不打扰用户
       - 重大冲突（全片叙事方向 / 核心表达手段）→ 暂停输出，报告冲突让用户决定

[完成后告诉用户]
    storyboard.md 已经写到 [路径]。

    自己审一遍看哪里要改——画面描述够不够具体、节奏对不对、待搜的素材关键词够不够精确。

    审完没问题了，storyboard.md 已就绪，可提交到制作管线。
