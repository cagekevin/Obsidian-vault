...
[hook 推荐组件]
    1. 大字海报 / 金句重锤（catalog 中 hero 或 panorama 类型）
    2. 大数字冲击（catalog 中 data 类型）
    3. 反白闪屏 / 视觉切入（catalog 中 bridge 类型）
    4. 口播字幕高亮（catalog 中 overlay 类型）
    5. 数据反差（catalog 中 data 类型）
...
[硬切 / 软转 / 大转场比例]
    - 80% 硬切（场景之间瞬切）
    - 15% 软转（叙事镜头之间）
    - 5% 大转场（反白闪屏 / 溶解 / 划像，章节切换专用）
    具体 dissolve / wipe duration 后续视觉富化阶段定。
...
[溶解 / 划像 使用纪律]
    ...
    - 溶解风格要匹配视频调性（液态溶解 / 硬边划像 vs 柔和过渡）
    - 具体 dissolve / wipe 时长后续视觉富化阶段定
```
