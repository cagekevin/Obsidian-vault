...
def check_asset_exists(filename):
    ...
    ad = assets_dir()
    target = os.path.join(ad, basename)
...
def list_all_ref_versions(element_name, json_filename=None):
    ad = assets_dir()
    if not os.path.isdir(ad):
        return []
    if json_filename:
        prefix = os.path.splitext(json_filename)[0]
        return [f for f in sorted(os.listdir(ad)) if f.lower().endswith((".png", ".jpg", ".jpeg")) and f.startswith(prefix)]
    # 跟 export_to_lovart.py 完全一致的匹配逻辑
    ...
    if os.path.exists(pj_path):
        ...
        for el in pj.get("elements", []):
            if el.get("name") == element_name:
                en = el.get("name", "")
                ref_key = f"ref_{en.replace(' ', '_')}" if not el.get("is_background") else f"ref_bg_{en.replace(' ', '_')}"
                prefix = ref_key.lower()
                files = []
                for f in sorted(os.listdir(ad)):
                    if f.lower().endswith((".png", ".jpg", ".jpeg")) and f.lower().startswith(prefix):
                        files.append(f)
                return files
    return []
...
def build_ref_images(jdata):
    ...
    for rp in jdata.get("reference_image_paths", []):
        exists, disp = check_asset_exists(rp)
        images.append({
            "originalPath": rp,
...
def api_project():
            ...
            if f.endswith(".json"):
                ...
                if 0 <= idx < len(groups):
                    for el_name in groups[idx].get("element_refs", []):
                        all_refs.extend(list_all_ref_versions(el_name))
                seen = set()
                all_refs_uniq = []
                for r in all_refs:
                    if r not in seen:
    ...
    # 图片单帧（和故事板结构一致：完整 prompt 只读 + description 可编辑）
        ...
        for f in sorted(os.listdir(refs_single_dir)):
            if f.endswith(".json"):
                ...
                ref_items.append({
                    ...
                    "resolution": jdata.get("resolution", ""), "model": jdata.get("model", ""),
                    "referenceImages": ref_images, "allRefImages": list_all_ref_versions(el_name, f),
...
def api_generate():
    ...
    def run_generation():
        log = []
        try:
            log.append(f"[1/4] 已更新 project.json: {el_name} appearance")
            export_script = os.path.join(_trans_dir, "export.py")
            log.append(f"[2/4] running export.py...")
            ...
            if r.returncode != 0: TASKS[task_id] = {"status": "error", "log": log + [f"export.py failed: {r.stderr}"]}; return
            log.append(f"[2/4] export.py done")

            split_script = os.path.join(_trans_dir, "export_to_lovart.py")
            log.append(f"[3/4] running export_to_lovart.py --type={split_type}...")
            ...
            if r.returncode != 0: TASKS[task_id] = {"status": "error", "log": log + [f"export_to_lovart.py failed: {r.stderr}"]}; return
            log.append(f"[3/4] export_to_lovart.py done")

            single_dir = os.path.join(output_dir(), f"{split_type}_single")
            ...
            if selected_refs and os.path.exists(target_json):
                ...
                for sr in selected_refs:
                    full = os.path.join(assets_dir(), sr)
                ...
                if ref_paths:
                    ...
                    log.append(f"[3.5/4] 垫图覆盖为: {selected_refs}")

            if not os.path.exists(target_json):
                TASKS[task_id] = {"status": "error", "log": log + [f"JSON not found: {target_json}"]}; return

            gen_script = os.path.join(SKILLS_MAIN_DIR, "skills/工作效率类/W7-API链接/lovart-skill/run_image_generator.py")
            log.append(f"[4/4] running run_image_generator.py {json_filename}...")
            r = subprocess.run([sys.executable, gen_script, target_json], capture_output=True, text=True, timeout=900, cwd=SKILLS_MAIN_DIR)
            log.append(f"[4/4] run_image_generator.py exit code: {r.returncode}")
            if r.stdout: log.append(r.stdout)
            if r.stderr: log.append(r.stderr)

            TASKS[task_id] = {"status": "done", "log": log, "returnCode": r.returncode}

        except subprocess.TimeoutExpired:
            TASKS[task_id] = {"status": "error", "log": log + ["Timeout exceeded"]}
        except Exception as e:
            TASKS[task_id] = {"status": "error", "log": log + [str(e)]}
...
def api_generate_scene():
    ...
    def run_generation():
        log = []
        try:
            log.append(f"[1/4] 已更新 project.json: {log_label}")
            _trans_dir = os.path.dirname(os.path.abspath(__file__))
            ...
            split_script = os.path.join(_trans_dir, "export_to_lovart.py")
            log.append(f"[3/4] running export_to_lovart.py --type={export_type}...")
            ...
            target_json = os.path.join(output_dir(), single_dir, json_filename)
            selected_refs = data.get("selectedRefs", [])
            if selected_refs and os.path.exists(target_json):
                ref_paths = []
                for sr in selected_refs:
            ...
            if not os.path.exists(target_json): TASKS[task_id] = {"status": "error", "log": log + [f"JSON not found: {target_json}"]}; return

            gen_script = os.path.join(SKILLS_MAIN_DIR, "skills/工作效率类/W7-API链接/lovart-skill/run_image_generator.py")
            r = subprocess.run([sys.executable, gen_script, target_json], capture_output=True, text=True, timeout=900, cwd=SKILLS_MAIN_DIR)
            TASKS[task_id] = {"status": "done", "log": log + [r.stdout, r.stderr], "returnCode": r.returncode}

        except subprocess.TimeoutExpired: TASKS[task_id] = {"status": "error", "log": log + ["Timeout exceeded"]}
        except Exception as e: TASKS[task_id] = {"status": "error", "log": log + [str(e)]}
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(os.path.join(PROJECT_DIR, "project.json")):
        print(f"错误: 项目目录下找不到 project.json: {PROJECT_DIR}")
        sys.exit(1)

    print(f"Lovart Web UI")
    print(f"  项目: {PROJECT_DIR}")
    print(f"  skills-main: {SKILLS_MAIN_DIR}")
    print(f"  http://127.0.0.1:{_args.port}")
