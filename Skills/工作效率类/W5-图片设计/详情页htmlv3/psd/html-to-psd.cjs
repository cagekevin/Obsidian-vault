#!/usr/bin/env node
/**
 * html-to-psd.cjs — 适配详情页htmlv3 架构
 *
 * 用法:
 *   node psd/html-to-psd.cjs <ProjectName> <ProjectRootDir>            (合并长图)
 *   node psd/html-to-psd.cjs <ProjectName> <ProjectRootDir> --split    (全部分屏)
 *   node psd/html-to-psd.cjs <ProjectName> <ProjectRootDir> --split --index=3 (指定第N屏)
 *
...
if (!projectName || !projectRootArg) {
    console.error('❌ Fail Fast: 请提供项目名称和项目目录。用法: node psd/html-to-psd.cjs <ProjectName> <ProjectRootDir> [--split] [--index=N]');
    process.exit(1);
}
const taskDir = path.resolve(projectRootArg);

// Node.js 自动向上查找 node_modules，无需手动添加路径
...
(async () => {
  ...
  const elements = await page.evaluate((opts) => {
    ...
    const results = [];
    const containerSelector = isSplit ? '.mod[id^="screen-"]' : '.page-wrapper';
  ...
  var containerSelector = isSplitMode ? '.mod[id^="screen-"]' : '.page-wrapper';
