...
A standard render tool is available: use `node <skill-assets>/render.cjs <task-dir> [selector...]`.

```bash
# Render all posters in a task folder
node 小红书微信图文/assets/render.cjs ./social-card-hiking

# Render only 3:4 posters
node 小红书微信图文/assets/render.cjs ./social-card-hiking xhs

# Render a specific poster by id
node 小红书微信图文/assets/render.cjs ./social-card-hiking xhs-01
```

The tool screenshots each `.poster` or `.cover` node individually, saves to `output/`, and prints dimensions. No separate render script needed in task folders.

If Playwright is not installed in the task folder:
```bash
cd <task-dir> && npm install playwright && npx playwright install chromium
```
