...
> 来源：[B站 - 李一帆AIGC - 别再硬续15秒了！AI长视频丝滑连贯的3个方法](https://www.bilibili.com/video/BV1qyjb6XEmX/)
> 
> 核心观点：90%的人做AI长视频翻车，不是模型不够强，而是不懂电影剪辑的逻辑。真正限制AI长视频制作的不是15秒，而是你在硬续长镜头。电影本来就不是一镜到底，而是靠镜头组接完成叙事。

### 方法一：切换景别和角度

**问题**：两段视频衔接时"怪怪的"，因为把相似的角度和景别连在了一起。

**操作**：
1. 第一段视频生成后，点击右下角**截取视频的尾帧**
2. 点击截取的图片，选择上方的**九宫格** → **多机位九宫格** → 点击生成
3. 从多个画面中选择你想要的画面，作为下一个视频的**首帧**
4. **选择原则**：要么选**不同的景别**，要么选**不同的角度**
5. 这样通过画面的切换，两段视频就能自然衔接在一起

### 方法二：在动作中衔接

**操作**：
1. 在第一段视频中找到**动作的起始动作**（比如"女生拿起咖啡"的那一刻）
2. 点击右下角**截取当前帧**
3. 点击刚才截取的画面，选择九宫格 → 多机位九宫格
4. 选择一个有**景别和角度变化**的图片
5. **关键**：不要用这个图片生成"喝咖啡"，而是在提示词中写：**"女生继续抬起咖啡杯，举到嘴边，轻轻喝一口，随后放下咖啡杯"**
6. 注意关键词是**"继续抬起"**，而不是"喝咖啡"——因为需要让**动作产生重叠**
7. 生成完成后，把两个动作对齐，**剪辑点放到动作进行到一半的位置**
8. 即使已经切换视频，观众也几乎察觉不到——因为咖啡杯始终在沿着同一个运动轨迹在移动

**原理**：利用**人类的视觉暂留现象**——人脸在看到画面后，影像会在视网膜停留约0.1到0.2秒。在角色或物品正在运动的瞬间切镜头，前后帧的运动轨迹会被大脑自动衔接，根本察觉不到断片。

### 方法三：分镜组接镜头

**核心思想**：不要硬续长镜头。在真正的电影中，导演会把故事拆成多个镜头。既然工具一次只能生成15秒，那就在**故事板阶段**把镜头衔接成**15秒之内能完成的小剧情**。

**操作**：
1. 把**剧本上传给AI**，点击加号 → 选择脚本 → 点击生成
2. AI会生成一个**标有时间的分镜头脚本**
3. 每次选择**15秒以内的脚本组**接在一起
4. 比如第一段：前四个镜头是4秒+3秒+3秒+4秒=14秒，正好在15秒之内
5. 把**人物设定图**和**场景参考图**同时喂给AI
6. 这样就可以生成第一段、第二段、第三段……
7. 每个片段只负责一个**剧情阶段**，既容易生成，组接起来也超过了15秒，更有**电影节奏**

### 与本SOP的关联

方法三"分镜组接"与SOP的"4宫格→视频"思路一致——先设计关键帧，再让AI补间。但本视频补充了方法二的**动作重叠衔接**，即过渡帧提示词用"继续"描述重叠动作，而非从零开始。将此技巧融入"视频连续性公式"可进一步提升长视频的丝滑度。
