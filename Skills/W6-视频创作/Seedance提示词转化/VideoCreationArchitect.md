...
**🚨 关键帧/故事板垫图规则（最重要）**：如果已有故事板垫图或关键帧图像作为参考，提示词**只需描写核心动作和摄像机运动**（人物做了什么、怎么做的、镜头怎么动），**不要重复描写画面背景、人物外观、光线、色调**——这些已经在垫图里了。重复描写会导致垫图和文字冲突，AI 不知道该听谁的。只写动作、运动方向、因果链、摄像机运动即可。
...
- [ ] **已添加负面清单**：no magic light effects, no glowing particles, no subtitles, no background music, no narration
