---
name: spec-rules
description: 填写 storyboard.md 模板时的字段约束、规格一致性校验、交付前自检清单。配合 templates/storyboard-template.md 使用——模板是骨架，本文件是「怎么填对」。起草和迭代 spec 前都要读。
---

# Video Spec 填写规则

`templates/storyboard-template.md` 是要填的骨架，本文件是每个字段的约束、规格一致性校验、和输出前的自检清单。
...
- **类型**：类型值必须属于当前 catalog 的 `shot_types` ID 列表
- **组件**：必须是 components-catalog.md 登记过的 ID；找不到 → 当前 catalog 的 fallback 值 + 在 § 9 登记
